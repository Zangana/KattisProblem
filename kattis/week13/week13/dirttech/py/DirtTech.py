...
lines = [[cut[0][0] * cut[1][1], cut[1][0] * cut[0][1], 8 * cut[0][1] * cut[1][1]],
         [mix[0][0] * mix[1][1], mix[1][0] * mix[0][1], 8 * mix[0][1] * mix[1][1]],
         [pac[0][0] * pac[1][1], pac[1][0] * pac[0][1], 8 * pac[0][1] * pac[1][1]],
         [1, 0, 0],
         [0, 1, 0]]
...
def sat(x1, x2, lines):
    return (x1 * lines[0][0] + x2 * lines[0][1] <= lines[0][2]) and (x1 * lines[1][0] + x2 * lines[1][1] <= lines[1][2]) and (x1 * lines[2][0] + x2 * lines[2][1] <= lines[2][2]) and (x1 >= 0) and (x2 >= 0)
